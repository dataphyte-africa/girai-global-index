...
function socialIconSvg(label: string) {
...
export function FooterSection({ content = footerDefaults }: { content?: FooterContent }) {
  return (
      ...
      <div className="max-w-6xl mx-auto px-6 pb-12">
        <div className="flex flex-col md:flex-row gap-10 md:gap-16">
          <div className="flex-1 max-w-xs">
            <Image src="/girai-logo-white.png" alt="GIRAI Logo" width={200} height={70} className="mb-4" />
            <div className="flex gap-3">
              {content.socialLinks.map((link) => (
                <SocialIcon key={link.label} href={link.href} label={link.label}>
                  {socialIconSvg(link.label)}
                </SocialIcon>
              ))}
            </div>
          </div>
