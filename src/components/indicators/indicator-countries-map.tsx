...
import { buildScoreRankMap } from "@/lib/performance-score";
import type { CountryRanking, DimensionScoreStats } from "@/lib/girai";

export interface IndicatorCountriesMapProps {
...
export function IndicatorCountriesMap({
  rankingData,
  indicatorSlug,
  indicatorName,
  scoreStats,
}: IndicatorCountriesMapProps) {
  ...
  const getScore = useCallback(
    (c: CountryRanking) => c.indicatorScores[indicatorSlug] ?? null,
    [indicatorSlug]
  );
  ...
  const getRank = useCallback(
    (c: CountryRanking) => rankMap.get(c.iso3) ?? null,
    [rankMap]
  );
      ...
      <div className="mx-auto max-w-6xl">
        <Tabs defaultValue="map" className="w-full">
          <div className="mb-8 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <h2 className="text-3xl md:text-4xl lg:text-5xl max-w-2xl font-semibold leading-tight">
